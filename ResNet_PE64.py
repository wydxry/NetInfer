...
class ResNet(nn.Module):
    def __init__(self, image_channels, image_size, block, num_blocks, num_classes=30):
        ...
        self.in_planes = 64
        self.embedding = Embedding(image_channels=image_channels, image_size=image_size, patch_size=2, dim=32)
    ...
    def forward(self, x):
        ...
        # print(out.shape)
        out = F.avg_pool2d(out, 8)  # 5, 2048, 1, 1
        # print(out.shape)
...
def test():
    ...
    model = ResNet50PE_64(img_channels, img_size)
    print("model: ", model)
...
def save_model_test():
    ...
    model = ResNet18PE_64(2, 128)
    image = torch.randn((1, 2, 128, 128))
    model.eval()
    traced_script_module = torch.jit.trace(model, image)
    traced_script_module.save("./model_0927_18_PE.pt")

    model = ResNet50PE_64(2, 128)
    image = torch.randn((1, 2, 128, 128))
    model.eval()
    traced_script_module = torch.jit.trace(model, image)
